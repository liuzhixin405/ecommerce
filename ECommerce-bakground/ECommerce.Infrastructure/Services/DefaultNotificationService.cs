...
namespace ECommerce.Infrastructure.Services
{
    public class DefaultNotificationService : INotificationService
    {
        ...
        public async Task SendInventoryAlertAsync(StockLockedEvent stockEvent)
        {
            try
            {
                _logger.LogInformation("Sending inventory alert for product {ProductId} with quantity {Quantity}", 
                    stockEvent.ProductId, stockEvent.Quantity);
                
                // 默认实现：记录通知发送日志
                await Task.Delay(100);
                
                _logger.LogInformation("Inventory alert sent successfully for product {ProductId}", stockEvent.ProductId);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Failed to send inventory alert for product {ProductId}", stockEvent.ProductId);
            }
        }
